...
// ============================================================
const BACKEND_URL = 'https://script.google.com/macros/s/AKfycbz9BMLlbRrpFYptSJZp8qRQqErbzliw1izKqhu2InNleO3EfWQvM-IbKvd_KMC6LSkN/exec';
...
const appState = {
  ...
  // Estado para la Proyección Mensual (Macro)
...
// Token de carga: si el usuario cambia de mes varias veces seguidas, solo la
// respuesta del último mes pedido puede pisar el estado (evita condiciones de
// carrera cuando una respuesta anterior llega tarde y de forma desordenada).
...
async function recargarEstadoMensual_() {
  ...
  try {
    // callBackendBackground ya se encarga del toast de sincronización, y ante un
    // error real muestra el aviso y reintenta recargar el estado del mes actual.
    const data = await callBackendBackground('getEstadoMensual', { year: year, month: month });
    if (miToken !== macroLoadToken_) return; // llegó tarde: el usuario ya cambió de mes de nuevo
    appState.macroData = data;
    renderMacroView();
  } catch (e) {
    // El error ya fue mostrado y manejado dentro de callBackendBackground.
  }
}
...
function changeMonth(delta) {
  ...
  // UI Optimista: el título del mes cambia al instante al tocar < o >, sin esperar
  // la respuesta del backend, para que la navegación nunca se sienta congelada.
  const monthDisplay = document.getElementById('current-month-display');
  if (monthDisplay) monthDisplay.textContent = NOMBRES_MESES[m] + ' ' + y;

  // Los datos (gastos, servicios, totales) se sincronizan en segundo plano.
  recargarEstadoMensual_();
}
